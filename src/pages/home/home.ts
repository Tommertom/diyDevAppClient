...
export class HomePage {
  ...
  constructor(public navCtrl: NavController,
    ...
    this.screenshotURI = this.base64Image;
  }
  ...
  takeScreenShot() {
    this.screenshot.URI(80)
      .then((onSuccess) => {
        this.screenshotURI = onSuccess['URI'];
        this.debugInfo = 'Screenshot ' + JSON.stringify(onSuccess, null, 2);
      }).catch(onError => {
        this.debugInfo = 'Screenshot error ' + JSON.stringify(onError, null, 2);
      });
  }
  ...
  doSMS() {
    this.sms.send('416123456', 'Hello world!')
      .then(res => {
        this.debugInfo = 'Send SMS ' + JSON.stringify(res, null, 2);
      })
      .catch(res => {
        this.debugInfo = 'Send SMS Error ' + JSON.stringify(res, null, 2);
      })
  }

  doSocialShare() {
    this.socialSharing.share('Body', 'Subject', '','http://ionic.io')
      .then(res => {
        this.debugInfo = 'Social share ok ' + JSON.stringify(res, null, 2);
      })
      .catch(res => {
        this.debugInfo = 'Social share not ok' + JSON.stringify(res, null, 2);
      })
  }
}
